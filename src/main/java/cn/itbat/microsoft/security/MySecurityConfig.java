    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .authenticationEntryPoint((request, response, authException) -> {
                    ...
                    out.write(objectMapper.writeValueAsString(BaseResultVo.error(1001, "未登录")));
                    out.flush();
                    out.close();
                })
                .and()
                ...
                .failureHandler((request, response, ex) -> {
                    ...
                    out.write(objectMapper.writeValueAsString(BaseResultVo.error(msg)));
                    out.flush();
                    out.close();
                })
                // 登录成功，返回json
                .successHandler((request, response, authentication) -> {
                    response.setContentType("application/json;charset=utf-8");
                    PrintWriter out = response.getWriter();
                    out.write(objectMapper.writeValueAsString(BaseResultVo.success(authentication)));
                    out.flush();
                    out.close();
                })
                .and()
                ...
                .accessDeniedHandler((request, response, ex) -> {
                    ...
                    response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                    PrintWriter out = response.getWriter();
                    out.write(objectMapper.writeValueAsString(BaseResultVo.error("权限不足")));
                    out.flush();
                    out.close();
                })
                .and()
                ...
                .logoutSuccessHandler((request, response, authentication) -> {
                    response.setContentType("application/json;charset=utf-8");
                    PrintWriter out = response.getWriter();
                    out.write(objectMapper.writeValueAsString(BaseResultVo.error("注销成功")));
                    out.flush();
                    out.close();
                })
                .and()
